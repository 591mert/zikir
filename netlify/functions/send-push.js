...
exports.handler = async (event) => {
  ...
  if (!PUBLIC_KEY || !PRIVATE_KEY) {
    return { statusCode: 500, body: JSON.stringify({ error: "VAPID anahtarları tanımlı değil." }), headers: { "Content-Type": "application/json", ...cors } };
  }
  const store = getStore("subscriptions");
  const list = await store.list();
  const isTest = (event.queryStringParameters && event.queryStringParameters.test === "1") || (event.test === "1");
  if (isTest) {
